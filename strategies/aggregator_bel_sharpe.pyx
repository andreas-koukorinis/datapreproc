...
class AggregatorBELSharpe(TradeAlgorithm):
    """Implement an aggregator strategy which combines multiple signals using Best expert learning approach based on past sharpe(as estimate of future sharpe) as the utility function
       ...
       **Weight of a singal = exp(past sharpe of signal)/sum_signal_weights

       This aggregator will recompute the relative allocations to the strategies every 3 month based on past 1 year of the signal
    ...
    def update_signal_allocations_using_bel_sharpe(self, _signals):
        ...
        self.signal_allocations = numpy.exp(self.signal_allocations) # Best expert learning assigns weights as exp of utility function (here expected sharpes)
    ...
    def on_events_update(self, events):
        ...
        if self.day >= self.last_day_sharpe_computed + self.sharpe_computation_interval:
            self.update_signal_allocations_using_bel_sharpe(self.signals)
