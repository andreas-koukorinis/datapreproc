...
def adjust_for_splits( path, products, product_type ):
    for product in products:
        ...
        if product_type == 'etf':
            ...
            if not os.path.isfile(split_file):
                df1.to_csv(path+product+'_split_adjusted'+'.csv',index=False)
                continue
            ...
            if not os.path.isfile(split_file):
                df.to_csv(path+product+'_split_adjusted'+'.csv',index=False)
                continue
            df2 = pd.read_csv(split_file,names=['date','new','old'])
            split_factor = 1.0
            for index, row in df2.iterrows():
