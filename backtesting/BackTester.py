...
class BackTester:
    def __init__(self,product,portfolio,performance_tracker,conversion_factor,positions_file):
    ...
    def updatePendingOrders(self,book,date,track,is_settlement):         
        ...
        if(is_settlement and self.product[-1]=='1'):
            self.switch_positions_on_settlement(self.product)

    def switch_positions_on_settlement(self,product):
        p1 = product
    ...
    def printOrders(self,typ,orders):
        if(len(orders)==0):
            return
        if(typ==0):
            s = 'ORDER PLACED : datetime:%s product:%s amount:%f'%(orders['dt'],orders['product'],orders['amount'])
        else:
            s = 'ORDER FILLED : '
            for order in orders:
