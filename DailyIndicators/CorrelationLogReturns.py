...
class CorrelationLogReturns( IndicatorListener ):
    ...
    def __init__(self, identifier, _startdate, _enddate, _config):
        ...
        _portfolio = params[1]
        self.products = sorted(get_products_from_portfolio_string(_portfolio))
    ...
    def get_unique_instance(identifier, _startdate, _enddate, _config):
        ...
        _products = sorted(get_products_from_portfolio_string(_identifier[1]))
        _sorted_identifier = _identifier[0] + '.' + make_portfolio_string_from_products(_products) + '.' + _identifier[2]
    ...
    def on_indicator_update(self, identifier, daily_log_returns_dt):
        ...
        if current_date > self.date:
            # this is the first update opf a new day
        ...
        if self.logret_matrix.shape[0] > self.period:
    ...
    def recompute ():
        if not self.correlation_matrix_already_computed:
            # TODO compute correlation matrix, covariance matrix, stdev array
