...
class TargetRiskExtendedEqualRiskContributionLogmax(SignalAlgorithm):
    ...
    def init(self, _config):
        ...
        self.exp_return_computation_history = ['63']
        self.exp_return_computation_interval = 5
    ...
    def process_param_file(self, _paramfilepath, _config):
        super(TargetRiskExtendedEqualRiskContributionLogmax, self).process_param_file(_paramfilepath, _config)
            ...
            if len(_model_line_words) >= 3:
                if _model_line_words[0] == 'Default':
                    if _model_line_words[1] == 'StdDevIndicator':
                        ...
                        _computation_words = _model_line_words[2:]
                        if len(_computation_words) == 2:
                            self.exp_return_computation_interval = int(_computation_words[0])
                            self.exp_return_computation_history = _computation_words[1:]
                    ...
                    if _product in self.products:
                        if _model_line_words[1] == 'StdDevComputationParameters':
                            ...
                            if len(_computation_words) == 2:
                                #set the refreshing interval to the minimum of current and previous values
                                self.exp_return_computation_interval = min(self.exp_return_computation_interval, int(_computation_words[0]))
                                _map_product_to_exp_return_computation_history[_product] = _computation_words[1:]                                
