...
class OrderManager():
    ...
    def print_placed_order(self, order):
        s = 'ORDER PLACED ON %s : id: %d product:%s amount:%0.2f' % ( order['dt'].date(), order['id'], order['product'], order['amount'] )
        text_file = open( self.positions_file, "a" )
        text_file.write("%s\n" % s)
        text_file.close()
    ...
    def print_cancelled_order(self, current_dt, order):
        s = 'ORDER CANCELLED ON %s : datetime:%s id: %d product:%s amount:%0.2f' % ( current_dt.date(), order['dt'], order['id'], order['product'], order['amount'] )
        text_file = open( self.positions_file, "a" )
        text_file.write("%s\n" % s)
        text_file.close()
