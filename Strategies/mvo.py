...
class MVO(TradeAlgorithm):
    ...
    def efficient_frontier(returns, covariance, leverage, risk_tolerance, max_allocation=0.5, min_allocation=0.0):
        """ Function that calculates the efficient frontier
            ...
            Args:
                ...
                max_allocation(float) - maximum weight to be allocated to one security
                                        (set low threshold to diversify)
                min_allocation(float) - minimum weight to be allocated to one security
                                        (set too high and portfolio won't be diversified)                        
            Returns:
        ...
        # Optimize
        portfolios = qp(Dmat, mu*dvec, Amat, bvec)['x']
        return portfolios
    ...
    def init(self, _config):
        ...
        if _config.has_option('Strategy', 'max_allocation'):
            self.min_allocation = _config.getfloat('Strategy', 'max_allocation')
        # Set minimum allocation
        self.min_allocation = 0.0
        if _config.has_option('Strategy', 'min_allocation'):
            self.min_allocation = _config.getfloat('Strategy', 'min_allocation')
        ...
        self.exp_return_computation_history = 21
        if _config.has_option('Strategy', 'exp_return_computation_history'):
            self.exp_return_computation_history = _config.get('Strategy', 'exp_return_computation_history').split(',')
